'''
@ Author: Yue 
@ Date: 2022-06-21 07:58:56
@ LastEditTime: 2022-09-27 08:27:16
@ FilePath: /gsy/FedGR/algorithms/fedrs.py
@ Description: 
...
class FedRS():
    ...
    def update_local(self, r, model, train_loader, test_loader, dist):
        ...
        ones_tensor = torch.ones_like(dist)
        #print("origin dist: ",dist)
        
        #TODO missing normalization
        #dist = dist / dist.max()
        #TODO ---------------begin test CrossEntropyLoss（weight = gamma） --------------
        padding_tensor = torch.tensor([1.]*10)
        #dist = torch.where(dist == 0, padding_tensor,dist)
        #dist = 1/ dist
        #---------------end test code--------------
        #dist = torch.where(dist > 0 , ones_tensor, dist)
        ...
        for t in range(n_total_bs + 1):
            ...
            # dist = torch.where(dist > 0 , ones_tensor, dist)
            #TODO Original Restricted Softmax
            ...
            #TODO without weighted Cross EntropyLoss
